...
const minterMSPID = "u0t790x9ia" //"Org1MSP" oorigin
...
func (s *SmartContract) TransferFrom(ctx contractapi.TransactionContextInterface, sender string, recipient string, id uint64, amount uint64) error {
	...
	if operator != sender {
		...
		if !approved { // TODO fix this
			// return fmt.Errorf("caller is not owner nor is approved. Op: %v, sender: %v", operator, sender)
			fmt.Print("WARN: Caller is not owner nor is approved. Op: %v, sender: %v", operator, sender)
		}
	}
...
func removeBalance(ctx contractapi.TransactionContextInterface, sender string, ids []uint64, amounts []uint64) error {
	...
	for _, tokenId := range necessaryFundsKeys {
		...
		if partialBalance < neededAmount {
			return fmt.Errorf("sender has insufficient funds for token %v, needed funds: %v, available fund: %v, PART_BALANCE=%v, balancePrefix=%v", tokenId, neededAmount, partialBalance, partBalAmount, balancePrefix)
